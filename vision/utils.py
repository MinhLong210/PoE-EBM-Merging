...
def prepare_data(config, device='cuda'):
    ...
    #-------------------------------------------------
        ...
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)
    elif data_config['type'] == 'fashionmnist':
        from dataset.fashionmnist import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)

    elif data_config['type'] == 'cifar10':
        from dataset.cifar10 import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)

    elif data_config['type'] == 'cifar100':
        from dataset.cifar100 import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)

    elif data_config['type'] == 'pets':
        from dataset.pets import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)

    elif data_config['type'] == 'flowers':
        from dataset.flowers import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)

    elif data_config['type'] == 'food':
        from dataset.food import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)
    else:
...
def strip_model_prefix(state_dict):
    ...
    for k, v in state_dict.items():
        if k.startswith("model.visual."):
            new_key = k.replace("model.visual.", "", 1)
            new_state_dict[new_key] = v
    return new_state_dict

def get_model_from_sd_fft(state_dict, base_model, pretrained=False):
    model = deepcopy(base_model)
    if pretrained:
        model.load_state_dict(state_dict, strict=True)
    else:
        clean_sd = strip_model_prefix(state_dict) # adapt state_dict's keys to base_model's
        model.load_state_dict(clean_sd, strict=True)
    return model.float().cuda()

def prepare_oc_vit(config, device):
    bases = []
    # Load pretrained model (both visual and text towers)
    # base_model, preprocess = clip.load(config['oc_name'], device, jit=False)
    # import pdb; pdb.set_trace()
    base_model, train_preprocess, eval_preprocess = open_clip.create_model_and_transforms(
        config['oc_name'],
        pretrained="openai"
    )
    config['train_preprocess'] = train_preprocess
    config['eval_preprocess'] = eval_preprocess

    # Saved load
    torch.save(base_model.state_dict(), f"{config['pretrained_model_dir']}/{config['pretrained_name']}.pt")
    
    model_paths = config['bases']
    for j, model_path in enumerate(model_paths):
        print(f"loading model #{j}")
        try:
            assert os.path.exists(model_path)
            state_dict = torch.load(model_path, map_location=torch.device(device))
            model = get_model_from_sd_fft(state_dict, base_model.visual)
            bases += [model]
        except:
            import pdb; pdb.set_trace()

    # Reload pretrained_model
    pre_trained_state_dict = torch.load(f"{config['pretrained_model_dir']}/{config['pretrained_name']}.pt", map_location=torch.device(device))
    new_model = get_model_from_sd_fft(pre_trained_state_dict, base_model, pretrained=True)
...
def prepare_experiment_config(config):
    ...
    if len(models['bases']) > 0 and hasattr(models['bases'][0], 'train_preprocess'):
        ...
        try:
            if isinstance(config['dataset'], list) and len(models['bases']) != len(config['dataset']):
                for dataset_config in config['dataset']:
                    dataset_config['train_preprocess'] = models['config']['train_preprocess']
                    dataset_config['eval_preprocess'] = models['config']['eval_preprocess']
                ...
                config['dataset']['train_preprocess'] = models['config']['train_preprocess']
                config['dataset']['eval_preprocess'] =  models['config']['eval_preprocess']
        
        except:
            dataset_config['train_preprocess'] = None
            dataset_config['eval_preprocess'] = None
...
# Fixed annotation function
def add_value_annotations(ax, matrix, fmt='.2f'):
    """Add value labels to each cell in the heatmap"""
    n_rows, n_cols = matrix.shape
    for i in range(n_rows):
        for j in range(n_cols):
            val = matrix[i, j]
            text_color = 'white' if val > 0.7 else 'black'  # Good contrast
...
def compute_norm(merged_weights):
    ...
    for layer_key in layers:
        matrix = merged_weights[layer_key]
        l1_norm_sum += torch.norm(matrix, p=1).item()
        l2_norm_sum += torch.norm(matrix, p=2).item()
        inf_norm_sum += torch.norm(matrix, p=float('inf')).item()
...
def preprocess_ftms_to_base_sd(ftms_sd, arch):
    """
    Convert fused ftms state_dict into base CLIP-style state_dict
    with explicit weight / bias parameters.

    Skips conv, ln_pre, ln_post.
    """
    new_sd = {}

    for k, v in ftms_sd.items():

        # # ---- skip explicitly ----
        # if any(s in k for s in ["conv", "ln_pre", "ln_post"]):
        #     continue

        if k == "conv1":
            # (768, 3072) → (768, 3, 32, 32)
            W = ftms_sd[k]
            new_sd["conv1.weight"] = W.view(768, 3, 32, 32) if arch.startswith("ViT-B") else W.view(1024, 3, 14, 14)
            continue

        if "ln" in k:
            # v: (d, d+1) = [diag(gamma) | beta]
            v = ftms_sd[k]
            d = v.shape[0]
            assert v.shape[1] == d + 1

            gamma = torch.diag(v[:, :d])   # extract diagonal
            # gamma = torch.diagonal(gamma)  # (d,)
            beta = v[:, d]                 # (d,)

            new_sd[f"{k}.weight"] = gamma
            new_sd[f"{k}.bias"] = beta
            continue

        # ---- LayerNorm (already separate) ----
        if k.endswith(".weight") or k.endswith(".bias"):
            new_sd[k] = v
            continue

        # ---- Attention in_proj (QKV fused) ----
        if k.endswith("attn.in_proj"):
            # v: (3d, d+1)
            d3, d1 = v.shape
            d = d3 // 3
            assert d1 == d + 1, f"Unexpected shape for {k}: {v.shape}"

            W = v[:, :d]     # (3d, d)
            b = v[:, d]      # (3d,)

            base = k.replace("attn.in_proj", "attn.in_proj")
            new_sd[base + "_weight"] = W
            new_sd[base + "_bias"] = b
            continue
        ...
        if k.endswith("c_fc"):
            ...
            # base = k.replace("attn.in_proj", "attn.in_proj")
            base = k
            new_sd[base + ".weight"] = W
            new_sd[base + ".bias"] = b
            continue

        # ---- c_fc layers ----
        if k.endswith("c_proj"):
            # v: (d, 4d+1)
            d, d4 = v.shape
            assert d4 == 4*d + 1, f"Unexpected shape for {k}: {v.shape}"
        ...
        # out_proj, etc.
        if v.ndim == 2 and v.shape[1] == v.shape[0] + 1:
            d = v.shape[0]
        ...
        # ---- Already square linear (no bias folded) ----
        if v.ndim == 2 and v.shape[0] == v.shape[1]:
            new_sd[k + ".weight"] = v
            continue

        # ---- Anything else is unexpected ----
        raise ValueError(f"Unhandled parameter {k} with shape {v.shape}")
