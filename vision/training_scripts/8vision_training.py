...
def train_functional(training_config = None):
    ...
    for dataset_name, loader_dict, class_vectors in tqdm(zip(dataset_names, data_loaders, all_clip_encodings)):
        ...
        save_path = os.path.join(
            model_save_dir, 
            f"{training_config['dataset']}_lr_{training_config['lr']}_wd_{training_config['wd']}_max_steps_{training_config['max_steps']}_early_stopping_patience_{training_config['early_stopping_patience']}.pt"
            # rescaling_weights{training_config['rescaling_weights']}.pt"
        )
...
if __name__ == "__main__":
    ...
    MODEL_SAVE_DIR = "LoRA_checkpoints"                                 # Path to save the model
